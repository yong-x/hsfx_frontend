<template>
	<div class="my-content"> 		
		<!-- 头部导航区 -->
		<van-nav-bar id="topNav" title="我发布的托管提供信息" @click-left="$router.go(-1)">
			<template #left>
		...
		<!-- 主体内容区 -->
			...
			@load="onLoad">
				...
				<van-card v-for="(item,index) in list" :desc="item.trust_detail" :price="item.price_monthly"  :title="item.trust_title" :thumb="getRealImgSrc(item.imglist[0])" :key="item.trustid" @click="toTrustingDetail(item)">
					<template #tags>
					...
					<template #footer>
					    <van-button @click.stop="editTrust(item)" type="info" size="mini">编辑</van-button>
					    <van-button @click.stop="deleteTrust(item.trustid,index)" type="danger" size="mini">删除</van-button>
...
<script>
	export default{
		...
		methods: {
			...
			editTrust(trust){ //编辑发布的托管需求（编辑与添加共用一个组件）
				this.$router.push({name:'addtrusting',params:{trust:trust}})
			}			
		},
