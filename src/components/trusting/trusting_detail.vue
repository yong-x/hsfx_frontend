<template>
	<div class="my-content">
		...
		<van-swipe :autoplay="3000" indicator-color="white" width="100%" height="300">
		  <van-swipe-item  v-for="(image, index) in trusting.imglist" :key="index">
		    <img style="width: 100%; height: 100%;" v-lazy="image" />
		  </van-swipe-item>
		</van-swipe>
		<!-- 发布人信息 -->
		<van-row class="publisher-Info" type="flex" justify="space-between" >
		  <van-col span="6"><span class="publisher_username">{{trusting.trusting_publisher}}</span></van-col>
		...
		<van-row class="trusting-baseInfo">
		  ...
		  <van-col span="8">
			  ...
			  <div class="desc-content">{{trusting.trusting_address}}</div>
		  </van-col>
		</van-row>
		<!-- 标签 -->
		...
		<div class="trusting-detail">
			...
			<p class="detail-content">{{trusting.trusing_detail}}</p>			
			<div class="detail-address">
				<a :href="'http://api.map.baidu.com/geocoder?address='+trusting.trusting_address+'&output=html&src=webapp.baidu.openAPIdemo'">
					<van-icon  class="iconfont" class-prefix="icon" name="dingwei" />  {{trusting.trusting_address}}
				</a>				
			</div>
		</div>
